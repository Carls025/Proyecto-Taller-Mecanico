...
<a href="{{ route('turnos.create') }}" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 mb-4 inline-block">➕ Agregar Turno</a>
        ...
        @foreach($turnos as $turno)
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-2 border flex space-x-2">
                    <a href="{{ route('turnos.edit', $turno) }}" class="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600">✏ Editar</a>
                    <form action="{{ route('turnos.destroy', $turno) }}" method="POST" onsubmit="return confirm('¿Seguro que deseas eliminar este turno?')">
                        @csrf
                        @method('DELETE')
